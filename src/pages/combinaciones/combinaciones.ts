...
import { EspecialesPage } from '../especiales/especiales';
...
export class CombinacionesPage {
  ...
  constructor(public navCtrl: NavController, 
          ...
          let cuponesPromo =  navParams.get("cupones");
         ...
         if( cuponesPromo != null ){
            cuponesPromo.forEach(cuponPromo => {
              this.cupones.push(cuponPromo)
            });
          }
  ...
  escucharEventos(){
    ...
    this.events.subscribe('nuevo-cupon', cuponesArray => {
      
      cuponesArray.forEach(cupon=>{
    ...
    this.events.subscribe('nuevo-adicional', adicionales => {
      /*if(this.combinacion.adicionales.length==0){
        this.combinacion.adicionales.push(adicionales);
      }else{*/
        adicionales.forEach(adicional => {
  ...
  ngOnDestroy(){
    ...
    this.events.unsubscribe('nuevo-cupon');
  ...
  /** Este metodo abre la pagina de adicionales, con el objetivo de anadir un nuevo adicional al resumen */
  anadirCupon(fab: FabContainer){
    fab.close();
    this.navCtrl.push(EspecialesPage , {
      objetivo : "nuevo-cupon",
  ...
  disminuirCantidadCupon(elemento) {
    elemento.cantidad--;
    //this.actualizarPrecioTotal();
  }
  ...
  aumentarCantidadCupon(elemento) {
    elemento.cantidad++;
    //this.actualizarPrecioTotal();
  }
  ...
  actualizarPrecioTotal(){
    ...
    //console.log("combinaciones pizza aquí------------------------------------------------------------>");
    //console.log(this.combinacion.pizzas)
    if (this.combinacion.pizzas.length > 0) {
      ...
      //this.mostrarMensajepopup("Aqui estamos revisando masa-costo,borde-costo,ingredientes-longitud",this.combinacion.pizzas[0].masa.costo+","+this.combinacion.pizzas[0].borde.costo+","+this.combinacion.pizzas[0].ingredientes[0].costoBase+"-"+this.combinacion.pizzas[0].ingredientes[0].porcion.valor);
    } else {
      totalPizzas = 0;
    }
  ...
  /*  if (this.promocionales.length != 0) {
    ...
    //console.log("combinaciones adicionales aquí------------------------------------------------------------>");
    //console.log(this.combinacion.adicionales)
    if (this.combinacion.adicionales.length != 0) {
    ...
    //console.log(JSON.stringify(this.tradicionales));
     ...
     //console.log("combinaciones tradicionales aquí------------------------------------------------------------>");
  ...
  /*
    if (this.tradicionalesPromo.length != 0) {
     ...
     //console.log("combinaciones combos aquí------------------------------------------------------------>");
    //console.log(this.combos)
    if (this.combos.length != 0) {
      this.combos.forEach((combo) => {
        if(combo.ID == 0 ){
          let bordeCost = 0
          totalCombos += Number(combo.COSTO)
          /*if(combo.NUM_PIZZAS==1){
            totalCombos += Number(combo.COSTOS[0]) 
          }else if(combo.NUM_PIZZAS==2){
            totalCombos += Number(combo.COSTOS[0]) + Number(combo.COSTOS[1]) 
          }else if(combo.NUM_PIZZAS==3){
            totalCombos += Number(combo.COSTOS[0]) + Number(combo.COSTOS[1]) + Number(combo.COSTOS[2])
          }else if(combo.NUM_PIZZAS==4){
            totalCombos += Number(combo.COSTOS[0]) + Number(combo.COSTOS[1]) + Number(combo.COSTOS[2]) + Number(combo.COSTOS[3]) 
          }else{
            totalCombos += Number(combo.COSTOS[0]) + Number(combo.COSTOS[1]) + Number(combo.COSTOS[2]) + Number(combo.COSTOS[3]) + Number(combo.COSTOS[4])
          }*/
 ...
 /** Este metodo elimina una pizza creada del resumen @param pizza */
  ...
  /** Este metodo elimina un combo del resumen @param cupon */
  eliminarCupon(cupon){
    let index = this.cupones.map(function (x) { return x.id; }).indexOf(cupon.id);
  ...
  aCarrito(){
    if(this.costoTotal!=0){
      ...
      this.mostrarMensaje(Constantes.DIA_INCORRECTO,Constantes.SOLOCUPONES);
    }
