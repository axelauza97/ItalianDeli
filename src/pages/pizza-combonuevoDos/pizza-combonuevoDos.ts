...
export class PizzaComboNuevoDosPage {
  ...
  siguiente() {
    ...
    if (this.pizzaUno.ingredientes.length == 0) {
      ...
      return;
        ...
        //Little
        this.costoCombo = this.navParams.get('costo');
          this.pizzaUno.costo = (this.costoCombo + this.bordeElegidoChoice.costo) /2;
  ...
  abrirCombinaciones() {
    ...
    if(this.combo['PIZZAS'].length>2){//si se regresa con el boton atras 
      ...
      this.combo['PIZZAS'] = [];
      this.combo['PIZZAS'].push(pizzascpy[this.combo['PIZZAS'].length-1]);
      this.combo['PIZZAS'].push(pizzascpy[this.combo['PIZZAS'].length-2]);
    }else if(this.combo['PIZZAS'].length==1){
      this.combo['PIZZAS'].push(this.pizzaUno);
    }
    this.combo = this.navParams.get("combo");
    //this.pizzaUno.cantidad=30;
    console.log("combo pizza uno --------------------------------------->");
    console.log(this.pizzaUno);
    console.log(this.combo);
    //this.combo['PIZZAS'].push(this.pizzaUno);
    /*if(this.diaPromo == this.combos[4].NOMBRE){
